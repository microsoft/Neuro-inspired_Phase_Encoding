...
class DinoVisionTransformer(nn.Module):
    ...
    def set_analysis_mode(self, mode: bool) -> None:
        """
        Recursively enable/disable attention analysis mode in all submodules.
        """
    ...
    def get_analysis_results(self) -> Dict[str, float]:
        """
        Aggregate analysis metrics from all attention layers.
        """
        results = {}
        layer_metrics = []
        ...
        for i, block in enumerate(self.blocks):
            # Handle BlockChunk (if self.chunked_blocks)
            if self.chunked_blocks and isinstance(block, nn.ModuleList):
                for sub_block in block:
                    # check for 'attn' (Block) or 'attn' inside nested block
            ...
            # Standard Block or NestedTensorBlock
