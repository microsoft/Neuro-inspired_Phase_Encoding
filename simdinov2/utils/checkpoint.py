# checkpointer from fvcore with some modifications
    ...
    def step(self, iteration: int, **kwargs: Any) -> None:
        ...
        if (iteration + 1) % self.period == 0:
            ...
            if self.max_to_keep is not None:
                ...
                while len(self.recent_checkpoints) > self.max_to_keep:
                    file_to_delete = self.recent_checkpoints.pop(0)
                    if self.path_manager.isdir(file_to_delete):
                        if dist.is_main_process() and self.path_manager.exists(
                            file_to_delete
                        ) and not file_to_delete.endswith(f"{self.file_prefix}_final"):
