...
def get_args_parser(
    ...
    parser.set_defaults(
        ...
        learning_rates=[5e-4, 1e-3, 2e-3, 5e-3, 1e-2, 2e-2, 5e-2, 0.1],
...
class AttentiveClassifier(LinearClassifier):
    def __init__(self, out_dim, use_n_blocks, use_avgpool, concat_cls=False, num_classes=1000, num_heads=8):
        super().__init__(out_dim, use_n_blocks, use_avgpool, num_classes)
...
def setup_linear_classifiers(sample_output, n_last_blocks_list, learning_rates, batch_size, num_classes=1000, attentive=False, attentive_kwargs=dict()):
...
def run_eval_linear(
    ...
    n_last_blocks_list = [1, 4] #if attentive else [1,2,4]
    n_last_blocks = max(n_last_blocks_list)
    autocast_ctx = partial(torch.cuda.amp.autocast, enabled=True, dtype=autocast_dtype)
    feature_model = ModelWithIntermediateLayers(model, n_last_blocks, autocast_ctx)
    sample_output = feature_model(train_dataset[0][0].unsqueeze(0).cuda())
    model_num_heads = {"vit_small": 6, "vit_base": 12, "vit_so150m2": 12, "vit_large": 16, "vit_giant2": 24,
                       "vit_kope_small": 6, "vit_kope_base": 12, "vit_kope_large": 16, "vit_kope_giant": 24,
                       "vit_rope_small": 6, "vit_rope_base": 12, "vit_rope_large": 16, "vit_rope_giant2": 24,
                       "vit_rope_so150m": 14, "vit_rope_so150m2": 13, "vit_rope_so300m": 16, "vit_rope_so400m": 16,
                       "aimv2_rope_large_patch14_224": 8,
                       "slorn_small": 6, "slorn_base": 12, "slorn_large": 16,
                       "akorn_small": 6, "akorn_base": 12, "akorn_large": 16,
                       "hybrid_slorn_small": 6, "hybrid_slorn_base": 12, "hybrid_slorn_large": 16,
                       "hybrid_akorn_small": 6, "hybrid_akorn_base": 12, "hybrid_akorn_large": 16,
                       "hybrid_akorn_smallv2": 6, "hybrid_akorn_basev2": 12, "hybrid_akorn_largev2": 16}
    attentive_kwargs = {"num_heads": model_num_heads[cfg.student.arch], "concat_cls": attentive_concat_cls}
