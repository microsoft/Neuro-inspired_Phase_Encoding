...
from simdinov2.models.vision_transformer_kope import DinoVisionTransformerKoPE, vit_kope_base, vit_kope_large, vit_kope_small, vit_kope_giant
...
class MyViTKoPE(BaseModule):
    def __init__(self,
                 ...
                 drop_path_rate=0.1,
                 with_kope_phase=False,
                 **kwargs):
        super().__init__(init_cfg=init_cfg)
        self.model_name = model_name
        self.out_indices = out_indices
        self.with_kope_phase = with_kope_phase

        if model_name == 'vit_kope_base':
            self.model = vit_kope_base(img_size=img_size, patch_size=patch_size, drop_path_rate=drop_path_rate, **kwargs)
        elif model_name == 'vit_kope_large':
            self.model = vit_kope_large(img_size=img_size, patch_size=patch_size, drop_path_rate=drop_path_rate, **kwargs)
        elif model_name == 'vit_kope_small':
            self.model = vit_kope_small(img_size=img_size, patch_size=patch_size, drop_path_rate=drop_path_rate, **kwargs)
        elif model_name == 'vit_kope_giant':
            self.model = vit_kope_giant(img_size=img_size, patch_size=patch_size, drop_path_rate=drop_path_rate, **kwargs)
        else:
    ...
    def init_weights(self):
        ...
        if self.init_cfg.get('type') == 'Pretrained':
            ...
            print(f"Loading MyViTKoPE checkpoint from {checkpoint_path}")
    ...
    def forward(self, x):
        outs = self.model.get_intermediate_layers(
            ...
            norm=True,
            return_token_and_phase=self.with_kope_phase
        )
        return outs
