...
from simdinov2.utils.checkpoint import Checkpointer
...
from torch.distributed.checkpoint.state_dict import get_model_state_dict, get_optimizer_state_dict, set_model_state_dict, set_optimizer_state_dict
...
def get_fsdp_wrapper(model_cfg, modules_to_wrap=set()):
    ...
    device_mesh = init_device_mesh("cuda", (dist.get_global_size(),))
...
def rankstr():
    return f"rank_{dist.get_global_rank()}"
...
class FSDPCheckpointer(Checkpointer):
    def save(self, name: str, **kwargs: Any) -> None:
        ...
        #data["model"] = get_state_dict(self.model, self.optimizer)
...
class DCPCheckpointer(FSDPCheckpointer):
    checkpoint_future = None
    def state_dict(self):
        # this line automatically manages FSDP FQN's, as well as sets the default state dict type to FSDP.SHARDED_STATE_DICT
        data = {}
        data["model"] = get_model_state_dict(self.model)
        if "optimizer" in self.checkpointables:
            data["optimizer"] = get_optimizer_state_dict(self.model, self.checkpointables["optimizer"])
        return data
    ...
    def save(self, name: str, **kwargs: Any) -> None:
        ...
        torch.save(kwargs, os.path.join(path, f"kwargs.{rankstr()}.pth"))
        writer = FileSystemWriter(path, cache_staged_state_dict=True)
        self.checkpoint_future = DCP.async_save(data, storage_writer=writer, checkpoint_id=path)
    ...
    def load(self, path: str, checkpointables = None):
        ...
        if os.path.isdir(path):
            self.logger.info("[Checkpointer] DCP loading from {} ...".format(path))
            data = self.state_dict()
            DCP.load(data, checkpoint_id=path)
            set_model_state_dict(self.model, data["model"])
            if "optimizer" in self.checkpointables:
                set_optimizer_state_dict(self.model, self.checkpointables["optimizer"], data["optimizer"])
            data = torch.load(os.path.join(path, f"kwargs.{rankstr()}.pth"))
            return data
        else:
            with FSDP.state_dict_type(self.model, StateDictType.SHARDED_STATE_DICT):
