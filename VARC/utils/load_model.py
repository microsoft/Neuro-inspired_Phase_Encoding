...
from src.ARC_ViT_KoPE_EncDec import ARCViTKoPEEncDec, ARCViTKoPE2EncDec
from src.ARC_ViT_EncDec import ARCViTEncDec
...
def get_model_arch(args, train_dataset):
    ...
    elif args.architecture == "vit_kope_encdec":
        model = ARCViTKoPEEncDec(
            num_tasks=train_dataset.num_tasks,
            image_size=args.image_size,
            num_colors=args.num_colors,
            embed_dim=args.embed_dim,
            ...
            kope_learn_gamma=args.kope_learn_gamma,
            kope_base=args.kope_base,
        )
    elif args.architecture == "vit_kope_encdec2":
        model = ARCViTKoPE2EncDec(
            num_tasks=train_dataset.num_tasks,
            image_size=args.image_size,
            num_colors=args.num_colors,
            embed_dim=args.embed_dim,
    ...
    elif args.architecture == "vit_encdec":
        model = ARCViTEncDec(
            num_tasks=train_dataset.num_tasks,
            image_size=args.image_size,
            num_colors=args.num_colors,
...
def load_models(args, train_dataset, device, distributed, rank, local_rank):
    ...
    if resume_checkpoint and os.path.exists(resume_checkpoint):
        ...
        # Handle task token mismatch or skipping
        if args.resume_skip_task_token and "task_token_embed.weight" in state_dict:
            ...
            if missing and ((not distributed) or rank == 0):
                print(f"Skipped loading parameters: {sorted(missing)}")
            if unexpected and ((not distributed) or rank == 0):
    ...
    # 4. Initialize Optimizer, Scaler, Scheduler
    ...
    # 5. Restore Train State (Optimizer, Scaler, Scheduler, Epoch)
    resume_reset_optimizer = getattr(args, "resume_reset_optimizer", False)

    if checkpoint is not None:
        if "optimizer_state" in checkpoint and not resume_reset_optimizer:
            optimizer.load_state_dict(checkpoint["optimizer_state"])

        if "scaler_state" in checkpoint and scaler.is_enabled() and not resume_reset_optimizer:
            scaler.load_state_dict(checkpoint["scaler_state"])

        if "scheduler_state" in checkpoint and scheduler is not None and not resume_reset_optimizer:
