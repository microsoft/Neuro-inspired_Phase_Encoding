...
def add_resume_checkpoints(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to a pretrained model to initialize weights from (used only if no resume checkpoint is active).",
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--architecture", type=str, choices=("vit", "unet", "vit_kope_encdec", "vit_kope_encdec2", "vit_encdec"), default="vit", help="ViT, UNet, ViTKoPEEncDec, ViTKoPEEncDec2, or ViTEncDec architecture")
    ...
    parser.add_argument("--depth-decoder", type=int, default=4, help="Depth of decoder (only for ViTKoPE).")
    ...
    # KoPE specific arguments
    parser.add_argument("--no-kope-vo-rotation", dest="kope_vo_rotation", action="store_false", help="Disable KoPE VO Rotation.")
    parser.set_defaults(kope_vo_rotation=True)
    parser.add_argument("--no-kope-mix", dest="kope_mix", action="store_false", help="Disable KoPE Mix.")
    parser.set_defaults(kope_mix=True)
    parser.add_argument("--kope-mix-init-gain", type=float, default=0.1, help="Initial gain for KoPE mix.")
    parser.add_argument("--no-kope-mix-phase-norm", dest="kope_mix_phase_norm", action="store_false", help="Disable KoPE Mix Phase Norm.")
    parser.set_defaults(kope_mix_phase_norm=True)
    parser.add_argument("--kope-gamma", type=float, default=0.05, help="Gamma value for KoPE.")
    parser.add_argument("--no-kope-learn-gamma", dest="kope_learn_gamma", action="store_false", help="Disable learning Gamma in KoPE.")
    parser.set_defaults(kope_learn_gamma=True)
    parser.add_argument("--kope-base", type=int, default=20, help="Base value for KoPE frequencies.")

    parser.add_argument("--ttt-num-each", type=int, default=2)
    parser.add_argument("--vis-every", type=int, default=25)
    parser.add_argument("--output-dir", type=str, default="outputs", help="Output directory for predictions.")
